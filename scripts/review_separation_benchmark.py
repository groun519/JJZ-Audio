...
def main() -> int:
    parser = argparse.ArgumentParser(description="Open the blind separation review window.")
    parser.add_argument("review", type=Path)
    parser.add_argument("--theme", choices=("dark", "white"), default="dark")
    parser.add_argument(
        "--screenshot",
        type=Path,
        help="Render one background screenshot instead of starting the event loop.",
    )
    args = parser.parse_args()
    app = QApplication(sys.argv[:1])
    app.setStyleSheet(build_stylesheet(args.theme))
    window = SeparationBenchmarkReviewWindow(args.review)
