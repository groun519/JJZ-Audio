...
class SongListRowTests(unittest.TestCase):
    ...
    def test_row_expands_inline_transport_without_resizing_the_title_column(self) -> None:
        ...
        row.show()
        self.app.processEvents()
        collapsed_body_height = row.body_container.height()
        collapsed_badge_size = row.source_badge.size()
        ...
        self.assertEqual(row.body_container.height(), collapsed_body_height)
        self.assertEqual(row.source_badge.size(), collapsed_badge_size)
        self.assertEqual(row.preview_transport.height(), TRANSPORT_BUTTON_SIZE)
